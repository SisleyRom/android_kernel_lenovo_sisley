...
static void goodix_gesture_work_func(struct work_struct *work)
{
    ...
    {  
		...
		/*	case 'c':
			letter = 0x20;
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_C, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_C, 0);
			input_sync(ts->input_dev);
			break;
			case 'e':
				letter = 0x20;
            ...
            */
			case 'v':
				letter = 0x34;
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_V, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_V, 0);
			input_sync(ts->input_dev);
			break;
			case 'o':
				letter = 0x33;
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_O, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_O, 0);
			input_sync(ts->input_dev);
			break;
			/*
			case 'w':
		...
		letter = 0x20;
	/*	if(type == 0){
               	 input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_RIGHT, 1);//KEY_GESTURE_LR
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_RIGHT, 0);
                	input_sync(ts->input_dev);
		}
		else if(type == 1){
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_DOWN, 1);//KEY_GESTURE_LR
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_DOWN, 0);
                	input_sync(ts->input_dev);
		}
		else if(type == 2){
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_UP, 1);//KEY_GESTURE_LR
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_UP, 0);
                	input_sync(ts->input_dev);
		}
		else if(type == 3){
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_LEFT, 1);//KEY_GESTURE_LR
                	input_sync(ts->input_dev);
                	input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_LEFT, 0);
                	input_sync(ts->input_dev);
		}*/
                // clear 0x814B
                doze_buf[2] = 0x00;
	...
	/* remove this function -----lenovo-sw2 lixh10*/
			...
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_Z, 1);
			input_sync(ts->input_dev);
			input_report_key(ts->input_dev, KEY_GESTURE_SLIDE_Z, 0);
			input_sync(ts->input_dev);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
	...
	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_SLIDE_V);
	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_SLIDE_O);
...
//	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_SLIDE_RIGHT);
	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_SLIDE_Z);
//	input_set_capability(ts->input_dev, EV_KEY, KEY_GESTURE_SLIDE_C);
#if 0
