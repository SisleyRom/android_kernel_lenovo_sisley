...
int mms_report_gesture_event(struct mms_ts_info *info,  u8 *buf)
{
	u8 gesture = 0;
       ...
       gesture = buf[0]&0x1f;
	letter = 0x00;
	//printk("MMS200 GESTURE SWITCH IS %d", gesture);
	switch(gesture){
		case 1 :
		...
		{//slide wake up
			...
			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_Z, 1);
    			input_sync(info->input_dev);
    			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_Z, 0);
     			input_sync(info->input_dev);
			break;
		}
		case 5:
		...
		{//"V"
			...
			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_V, 1);
    			input_sync(info->input_dev);
    			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_V, 0);
     			input_sync(info->input_dev);
			break;
		}
		case 8:
		{//"O" 
			...
			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_O, 1);
    			input_sync(info->input_dev);
    			input_report_key(info->input_dev, KEY_GESTURE_SLIDE_O, 0);
     			input_sync(info->input_dev);
			break;
		}
		default:
...
/*Begin lenovo-sw, lixh10, 20140730, add for slide touch */
	...
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_SLIDE_V);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_SLIDE_O);
	input_set_capability(input_dev, EV_KEY, KEY_GESTURE_SLIDE_Z);
